...
<<<<<<< HEAD
    ...
    {
        $query = 'select invoice_total from ip_invoice_amounts where invoice_id = '.$invoice_id;

    return $this->db->query($query)->row()->invoice_total;
    }

    public function get_by_invoice($invoice_id)
    {
        $query = 'select * from ip_invoice_amounts where invoice_id = '.$invoice_id;
