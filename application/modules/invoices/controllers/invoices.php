...
<<<<<<< HEAD
  public function fiskaliziraj($invoice_id)
    {
        $CI = &get_instance();

        $CI->load->model('invoices/mdl_invoices');
        $CI->load->model('users/mdl_users');
        $CI->load->model('invoices/mdl_invoice_amounts');        
        $CI->load->model('invoices/mdl_items');

        $invoice = $CI->mdl_invoices->get_by_id($invoice_id);    
        $user = $CI->mdl_users->get_by_id($invoice->user_id);
        $invoice_items = $CI->mdl_items->where('invoice_id', $invoice_id)->get()->result(); 
        $this->load->library('Fiskalizacija' , array('invoice' => $invoice, 'invoice_items' => $invoice_items));
    	$uuid = $this->fiskalizacija->guidv4(openssl_random_pseudo_bytes(16));
        $zki = $this->fiskalizacija->get_zki();
        $XMLRequestType = 'RacunZahtjev';
        $datum_slanja =  date('Y-m-d H:i:s');
        $response = $this->fiskalizacija->get_jir($XMLRequestType,$uuid,$zki,$datum_slanja);
      
        if ($response["jir_ok"] == "true")
        { 
       
        $data = array (
		    'invoice_id' => $invoice_id,
		    'invoice_uuid' => $uuid,
		    'invoice_datum_slanja' => $datum_slanja,
            'invoice_zki' => $zki,
            'invoice_fiskalizirano' => "1",
            'invoice_jir' => $response["jir"],
            'is_read_only' => "1",
            'invoice_status_id' => "4"
            
        );
        
        /*Spremanje rezultata iz library Fiskalzacija u Bazu*/	
        ...
        else
        {
          ...
          $data = array (
		    ...
		    'invoice_datum_slanja' =>  $datum_slanja,
            'invoice_zki' => $zki,
            'invoice_fiskalizirano' => "0",
            'invoice_naknadna_dostava' => "1",
            'invoice_jir' => "",
            'invoice_greska' => $response["jir"],
             'is_read_only' => "NULL"
        ...
        /*Spremanje rezultata iz library Fiskalzacija u Bazu*/	
	    $this->mdl_invoices->fiskaliziranje($data);
